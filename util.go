...
func toIndentedJSON(value lua.LValue, visited map[*lua.LTable]bool, indent string) (data []byte, err error) {
	switch converted := value.(type) {
	case lua.LBool:
		data, err = json.MarshalIndent(converted, "", indent)
	case lua.LChannel:
		err = errChannel
	case lua.LNumber:
		data, err = json.MarshalIndent(converted, "", indent)
	...
	case *lua.LTable:
		...
		if obj != nil {
			...
			data, err = json.MarshalIndent(arr, "", indent)
		}
	case *lua.LUserData:
		// TODO: call metatable __tostring?
		err = errUserData
